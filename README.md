...
## Java toolchain integration tests

The integration tests that invoke `java`, `javac`, and `javap` are disabled by
default. Enable them explicitly when a JDK is available on `PATH`:

```bash
ANVIL_RUN_JAVA_INTEGRATION_TESTS=1 \
  dotnet test --filter "Category=JavaIntegration"
```

These tests verify JVM execution and strict bytecode verification, disassemble a
class built from scratch by AnvilASM, and round-trip control-flow bytecode
compiled by `javac`.
